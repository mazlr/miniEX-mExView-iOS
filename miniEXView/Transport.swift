...
final class TCPTransport: MiniEXTransport {
    ...
    func connect(host: String, port: UInt16) {
        ...
        c.stateUpdateHandler = { [weak self] state in
            switch state { case .ready: self?.onState?("Připojeno"); self?.receive(); case .waiting(let e): self?.onState?("Čekám: \(e.localizedDescription)"); case .failed(let e): self?.onState?("Chyba: \(e.localizedDescription)"); default: break }
        }; c.start(queue: queue); onState?("Připojuji…")
    }
    func send(_ data: Data) { connection?.send(content: data, completion: .contentProcessed { [weak self] error in if let error { self?.onState?("Chyba zápisu: \(error.localizedDescription)") } }) }
